...
get_node_editor_panning(PyObject* self, PyObject* args, PyObject* kwargs)
{
	...
	mvNodeEditor* editor = static_cast<mvNodeEditor*>(anode_editor);
	auto panning = editor->getPanning();

	return ToPyPair(panning.first, panning.second);
}

static PyObject*
reset_node_editor_panning(PyObject* self, PyObject* args, PyObject* kwargs)
{
	PyObject* node_editor_raw;
	float x;
	float y;

	if (!Parse((GetParsers())["reset_node_editor_panning"], args, kwargs, __FUNCTION__, &node_editor_raw, &x, &y))
		return ToPyBool(false);
	...
	if (anode_editor == nullptr)
	{
		mvThrowPythonError(mvErrorCode::mvItemNotFound, "reset_node_editor_panning",
			"Item not found: " + std::to_string(node_editor), nullptr);
		return GetPyNone();
	}
	...
	if (anode_editor->type != mvAppItemType::mvNodeEditor)
	{
		mvThrowPythonError(mvErrorCode::mvIncompatibleType, "reset_node_editor_panning",
			"Incompatible type. Expected types include: mvNodeEditor", anode_editor);
		return GetPyNone();
	}

	mvNodeEditor* editor = static_cast<mvNodeEditor*>(anode_editor);
	editor->resetPanning(x, y);
